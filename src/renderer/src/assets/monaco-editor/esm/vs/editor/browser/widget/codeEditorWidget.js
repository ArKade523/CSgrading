...
let CodeEditorWidget = (CodeEditorWidget_1 = class CodeEditorWidget extends Disposable {
    ...
    constructor(
        ...
        this._register(
            new EditorModeContext(this, this._contextKeyService, languageFeaturesService)
        )
        ...
        for (const action of EditorExtensionsRegistry.getEditorActions()) {
            ...
            const internalAction = new InternalEditorAction(
                ...
                action.alias,
                action.metadata,
                (_a = action.precondition) !== null && _a !== void 0 ? _a : undefined,
                () => {
        ...
        const isDropIntoEnabled = () => {
            return (
                !this._configuration.options.get(90 /* EditorOption.readOnly */) &&
    ...
    getConfiguredWordAtPosition(position) {
        ...
        return WordOperations.getWordAtPosition(
            ...
            this._configuration.options.get(129 /* EditorOption.wordSeparators */),
            position
        )
    }
    ...
    _attachModel(model) {
        ...
        const viewModel = new ViewModel(
            ...
            (callback) =>
                dom.scheduleAtNextAnimationFrame(dom.getWindow(this._domElement), callback),
        ...
        this._contextKeyService.bufferChangeEvents(() => {
            ...
            this._hasMultipleDocumentFormattingProvider.set(
                this._languageFeaturesService.documentFormattingEditProvider.all(model).length +
                    this._languageFeaturesService.documentRangeFormattingEditProvider.all(model)
                        .length >
                    1
            )
...
registerThemingParticipant((theme, collector) => {
    ...
    if (errorForeground) {
        collector.addRule(
            `.monaco-editor .${'squiggly-error' /* ClassName.EditorErrorDecoration */} { background: url("data:image/svg+xml,${getSquigglySVGData(errorForeground)}") repeat-x bottom left; }`
        )
    }
    const warningForeground = theme.getColor(editorWarningForeground)
    if (warningForeground) {
        collector.addRule(
            `.monaco-editor .${'squiggly-warning' /* ClassName.EditorWarningDecoration */} { background: url("data:image/svg+xml,${getSquigglySVGData(warningForeground)}") repeat-x bottom left; }`
        )
    }
    const infoForeground = theme.getColor(editorInfoForeground)
    if (infoForeground) {
        collector.addRule(
            `.monaco-editor .${'squiggly-info' /* ClassName.EditorInfoDecoration */} { background: url("data:image/svg+xml,${getSquigglySVGData(infoForeground)}") repeat-x bottom left; }`
        )
    }
    const hintForeground = theme.getColor(editorHintForeground)
    if (hintForeground) {
        collector.addRule(
            `.monaco-editor .${'squiggly-hint' /* ClassName.EditorHintDecoration */} { background: url("data:image/svg+xml,${getDotDotDotSVGData(hintForeground)}") no-repeat bottom left; }`
        )
    }
    const unnecessaryForeground = theme.getColor(editorUnnecessaryCodeOpacity)
    if (unnecessaryForeground) {
        collector.addRule(
            `.monaco-editor.showUnused .${'squiggly-inline-unnecessary' /* ClassName.EditorUnnecessaryInlineDecoration */} { opacity: ${unnecessaryForeground.rgba.a}; }`
        )
    }
})
