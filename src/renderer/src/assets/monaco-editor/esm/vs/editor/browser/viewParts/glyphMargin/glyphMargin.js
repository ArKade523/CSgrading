...
export class DedupOverlay extends DynamicViewOverlay {
    ...
    _render(visibleStartLineNumber, visibleEndLineNumber, decorations) {
        ...
        for (let i = 0, len = decorations.length; i < len; i++) {
            ...
            const endLineIndex =
                Math.min(d.endLineNumber, visibleEndLineNumber) - visibleStartLineNumber
            ...
            if (a.lineNumber === b.lineNumber) {
                if (a.lane === b.lane) {
                    if (a.zIndex === b.zIndex) {
                        if (b.type === a.type) {
                            if (
                                a.type === 0 /* GlyphRenderRequestType.Decoration */ &&
    ...
    prepareRender(ctx) {
        ...
        while (requests.length > 0) {
            ...
            const requestsAtLocation = requests.takeWhile(
                (el) => el.lineNumber === first.lineNumber && el.lane === first.lane
            )
