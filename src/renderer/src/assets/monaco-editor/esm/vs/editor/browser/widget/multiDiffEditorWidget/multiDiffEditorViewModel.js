...
export class MultiDiffEditorViewModel extends Disposable {
    constructor(_model, _instantiationService) {
        ...
        this._documents = observableFromEvent(
            ...
            /** @description MultiDiffEditorViewModel.documents */ () => this._model.documents
        )
        this.items = derivedWithStore(this, (reader, store) =>
            this._documents
                .read(reader)
                .map((d) => store.add(new DocumentDiffItemViewModel(d, this._instantiationService)))
