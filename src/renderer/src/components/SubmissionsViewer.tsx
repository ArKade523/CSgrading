...
function SubmissionsViewer() {
    ...
    const { submissions, selectedSubmission } = useSelector((state: RootState) => state.currentAssignment);
    
    return (
        <div id="submissions-viewer">
            <ul>
                {submissions.map((submission) => (
                    <li key={submission.submissionTime} className={`submission-list ${submission === selectedSubmission ? "active" : ""}`}>
